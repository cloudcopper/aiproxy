...
test_name "Load combined certificate file"
test_description "Tests that aiproxy can load both certificate and key from a single combined PEM file"
...
# Start aiproxy with combined cert file (both flags point to same file), logs go to file
timeout 3s ./aiproxy \
...
# Check that it attempted to load from combined file
grep -q "combined" "$LOG_FILE" || test_fail "Combined file loading not detected in logs"
test_execution_end
