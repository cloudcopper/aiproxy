...
test_name "Localhost blocking: ::1 (IPv6) blocked"
test_description "Tests that requests to ::1 IPv6 localhost address are blocked with HTTP 403 (SSRF protection)"
...
HTTP_STATUS=$(timeout 3s ./aiproxy \
  ...
  -- bash -c "NO_PROXY='' curl -s -w '%{http_code}' -o '$TEST_DIR/curl_output.txt' 'http://[::1]:8080/test'" || true)
